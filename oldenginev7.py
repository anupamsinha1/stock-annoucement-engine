...
def evaluate_with_claude(prompt):
    ...
    wait_times = [5, 15, 30, 60]  # Progressive delay for rate limits & bursts
...
def evaluate_with_gemini(prompt):
    ...
    wait_times = [15, 30, 60, 120]  # Progressive delays for high server demand
    ...
    for attempt, wait_time in enumerate(wait_times + [None]):
        try:
            ...
            if "503" in error_msg or "UNAVAILABLE" in error_msg or "429" in error_msg or "RESOURCE_EXHAUSTED" in error_msg:
                if wait_time is not None:
                    print(
                        f" [Gemini 503/429 Overload] High demand. Waiting {wait_time}s before Retry {attempt + 1}/4...")
                    time.sleep(wait_time)
                    continue
                else:
...
def run_sharded_sieve2_workers(hits, num_workers=4):
    ...
    counter = itertools.count()  # Tie-breaker for PriorityQueue tuples
    ...
    def claude_worker(worker_id):
        ...
        while not stop_event.is_set():
            ...
            if stage == 1:
                if cat_score_safe > 4:
                    print(
                        f" -> [Claude Worker-{worker_id} Catalyst Score {cat_score_safe}/10 > 4] Pushing {item['company']} to Gemini Queue (Priority 0 URGENT)...")
                    gemini_queue.put((0, next(counter),
                                      {'item': item, 'stage': 2, 'evals': evals, 'market_data': market_data,
                                       'forum_data': forum_data}))
                else:
                    print(
                        f" -> [Claude Worker-{worker_id} Catalyst Score {cat_score_safe}/10 <= 4] Finalizing {item['company']} as SINGLE_MODEL_IGNORE.")
                    ignore_tuples = [(att_id, item['company'], item['headline'], "IGNORE") for att_id in
                                     item['all_ids']]
    ...
    def gemini_worker(worker_id):
        ...
        while not stop_event.is_set():
            ...
            if stage == 1:
                if cat_score_safe > 4:
                    print(
                        f" -> [Gemini Worker-{worker_id} Catalyst Score {cat_score_safe}/10 > 4] Pushing {item['company']} to Claude Queue (Priority 0 URGENT)...")
                    claude_queue.put((0, next(counter),
                                      {'item': item, 'stage': 2, 'evals': evals, 'market_data': market_data,
                                       'forum_data': forum_data}))
                else:
                    print(
                        f" -> [Gemini Worker-{worker_id} Catalyst Score {cat_score_safe}/10 <= 4] Finalizing {item['company']} as SINGLE_MODEL_IGNORE.")
                    ignore_tuples = [(att_id, item['company'], item['headline'], "IGNORE") for att_id in
                                     item['all_ids']]
                    log_announcements_batch(ignore_tuples)
...
# ---------------------------------------------------------------------------
# 9. DUAL-EXCHANGE INGESTION MODULES (CLEAN URLS & SAFE SCRAPING)
# ---------------------------------------------------------------------------
