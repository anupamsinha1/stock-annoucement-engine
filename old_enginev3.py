...
def log_announcements_batch(decisions_list):
    """
    Bulk inserts triage decisions to avoid exhausting the database connection pool.
    Replaces the inefficient single-row insert for routine filings.
    """
        ...
        if "Bullishness Score:" in line:
            ...
            if digits:
                try:
                    ...
                    pass
    return None  # Changed from 5 to None to properly flag analysis failures
...
def evaluate_with_claude(prompt):
    ...
    try:
        ...
        # FIX: Claude now returns ThinkingBlocks natively. Safely extract only TextBlocks.
...
def run_tier2_consensus_audit(company, exchange, scrip_code, isin, headline, forum_text, market_metrics):
    ...
    **Bullishness Score:** <X/10 - with 1-sentence rationale>
    ...
    # Safe consensus calculation accounting for potential API failures
        ...
        try:
            ...
            if resp.status_code == 200:
                ...
                for item in table:
                    ...
                    # FIX: Enforce 'N/A' if ISIN_CODE is returned as an empty string
...
def process_single_announcement(item):
    ...
    # Note: Short-term cache logging is now handled by the batch function
...
def evaluate_and_dispatch_filings(unprocessed_items):
    ...
    # FIX: Write all DB rows in one bulk insert to prevent connection hanging
    if batch_cache_data:
