...
- Tier 3 Quant Audit: 20-day delivery volume surges & 50/200 DMA trend checks (YFinance)
...
def filter_unprocessed_announcements(filings):
    ...
    try:
        ...
        # Keep only items whose unique 'id' does not exist in the database
...
def log_announcement_decision(attachment_id, company, headline, decision):
    conn = get_db_connection()
    if not conn:
        return
    try:
        ...
        cursor.execute(
            ...
            """,
            (attachment_id, company, headline, decision)
        )
        conn.commit()
        conn.close()
    except Exception as e:
...
def fetch_market_metrics(scrip_code, exchange):
    """
    Dynamically routes Yahoo Finance queries based on the exchange.
    BSE numerical codes use '.BO' (e.g., 532667.BO).
    NSE alphabetical codes use '.NS' (e.g., RELIANCE.NS).
    """
    ...
    try:
        # Construct correct exchange suffix for yfinance
...
def fetch_live_nse_filings():
    """
    Simulates a browser session to clear NSE Cloudflare cookies, then queries the API.
    Converts raw JSON into the standardized dictionary format.
    """
    headers = {
    ...
    try:
        # Step 1: Prime the session cookies to bypass anti-scraping
        session.get("https://www.nseindia.com", timeout=10)

        # Step 2: Fetch Equities & SME announcements
...
def fetch_live_bse_filings(max_pages=100):
    """
    Dynamically pages through the BSE API until it hits the end of the day's payload.
    Converts raw JSON into the standardized dictionary format.
    """
    headers = {
...
def process_single_announcement(item):
    ...
    log_announcement_decision(item['id'], item['company'], item['headline'], "HIT")
...
def evaluate_and_dispatch_filings(unprocessed_items):
    ...
    for item in unprocessed_items:
        if item['id'] in hit_ids:
            ...
            log_announcement_decision(item['id'], item['company'], item['headline'], "IGNORE")
