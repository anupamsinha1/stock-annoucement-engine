...
- Tier 2 Deep-Dive: Sharded Priority Queues with Cross-Evaluation (Claude Sonnet 5 + Gemini 3.1 Pro)
...
parser.add_argument("--ignore-cache", "-f", action="store_true",
                    help="Bypass Supabase cache and re-evaluate all filings")
...
def group_filings_by_company(filings):
    ...
    for item in filings:
        ...
        if key not in grouped:
            grouped[key] = {
                'id': item['id'],  # Primary ID for ledger mapping
                'all_ids': [item['id']],
            ...
            # Only append the headline if it's uniquely descriptive
...
def run_tier1_batch_sieve(announcements):
    ...
    for i in range(0, len(announcements), chunk_size):
        ...
        items_payload = [
            {"index": idx, "exchange": ann['exchange'], "scrip": ann['scrip'], "company": ann['company'],
            ...
            for dec in parsed_decisions:
                ...
                if idx is not None and idx < len(chunk):
                    ...
                    if status == "HIT":
                        ...
                        print(
                            f" [REJECTED] {ann_item['company']} ({ann_item['exchange']}:{ann_item['scrip']}) -> {reason}")
...
def build_sieve2_prompt(item, market_data, forum_text):
    ...
    price_display = f"INR {market_data['price']}" if market_data[
                                                         'price'] > 0 else "Data Feed In Sync (Live quote unavailable - evaluate purely on strategic/economic merits)"
    mkt_cap_display = f"INR {market_data['market_cap_cr']} Cr" if market_data.get('market_cap_cr',
                                                                                  0) > 0 else "Not specified"
...
def evaluate_with_claude(prompt):
    """Executes deep qualitative reasoning via Claude Sonnet."""
        ...
        try:
            ...
            if "503" in error_msg or "UNAVAILABLE" in error_msg:
                if wait_time is not None:
                    print(f" [Gemini 503 Overload] High demand. Waiting {wait_time}s before Retry {attempt + 1}/3...")
...
# ---------------------------------------------------------------------------
# 6. SHARDED PRIORITY QUEUE WORKER POOL & CROSS-EVALUATION
# ---------------------------------------------------------------------------
def finalize_dual_evaluation(item, evals, market_data):
    ...
    # Log ALL associated filing IDs for this company as HIT to prevent future re-processing
    hit_tuples = [(att_id, item['company'], item['headline'], "HIT") for att_id in item['all_ids']]
    log_announcements_batch(hit_tuples)


def run_sharded_sieve2_workers(hits):
    """Shards Sieve-2 candidates into two worker queues (Claude & Gemini)."""
    if not hits:
        return

    import itertools
    counter = itertools.count()  # Fix: Tie-breaker for the PriorityQueue
    ...
    def claude_worker():
        ...
        while not stop_event.is_set():
            try:
                # Unpack the 3-item tuple (priority, seq, task)
            ...
            print(f"\n[Claude Worker Stage {stage}] Evaluating {item['company']} ({item['exchange']}:{item['scrip']})...")
            ...
            if stage == 1:
                if cat_score_safe > 4:
                    print(f" -> [Claude Catalyst Score {cat_score_safe}/10 > 4] Pushing {item['company']} to Gemini Queue (Priority 0 URGENT)...")
                    gemini_queue.put((0, next(counter), {'item': item, 'stage': 2, 'evals': evals, 'market_data': market_data, 'forum_data': forum_data}))
                else:
                    print(f" -> [Claude Catalyst Score {cat_score_safe}/10 <= 4] Finalizing {item['company']} as SINGLE_MODEL_IGNORE.")
                ...
                print(f" -> [Dual Evaluation Complete] {item['company']} finalized by Claude.")
    ...
    def gemini_worker():
        ...
        while not stop_event.is_set():
            ...
            print(f"\n[Gemini Worker Stage {stage}] Evaluating {item['company']} ({item['exchange']}:{item['scrip']})...")
            prompt = build_sieve2_prompt(item, market_data, forum_data)
            ...
            if stage == 1:
                if cat_score_safe > 4:
                    print(f" -> [Gemini Catalyst Score {cat_score_safe}/10 > 4] Pushing {item['company']} to Claude Queue (Priority 0 URGENT)...")
                    claude_queue.put((0, next(counter), {'item': item, 'stage': 2, 'evals': evals, 'market_data': market_data, 'forum_data': forum_data}))
                else:
                    print(f" -> [Gemini Catalyst Score {cat_score_safe}/10 <= 4] Finalizing {item['company']} as SINGLE_MODEL_IGNORE.")
                    ignore_tuples = [(att_id, item['company'], item['headline'], "IGNORE") for att_id in item['all_ids']]
                    log_announcements_batch(ignore_tuples)
                ...
                print(f" -> [Dual Evaluation Complete] {item['company']} finalized by Gemini.")
                finalize_dual_evaluation(item, evals, market_data)
                with completed_lock:
                    completed_count += 1
                    if completed_count >= total_items:
    ...
    t_claude = threading.Thread(target=claude_worker, daemon=True)
    t_gemini = threading.Thread(target=gemini_worker, daemon=True)
...
def build_telegram_message(company, exchange, scrip_code, isin, market_data, audit, all_links):
    ...
    # Append all relevant links for the grouped filing
...
def main():
    ...
    # Tier 2 Sharded Queue Execution
    if hits:
        run_sharded_sieve2_workers(hits)
